...
int getSizeRoot(struct node* n){
  if(n->leftChild != NULL){
    if(n->rightSibling != NULL){
      return n->size + getSizeRoot(n->leftChild) + getSizeRoot(n->rightSibling);
    }
    else{
      return n->size + getSizeRoot(n->leftChild);
    }
  }
  else{
    if(n->rightSibling != NULL){
      return n->size + getSizeRoot(n->rightSibling);
    }
    else{
      return n->size;
    }
